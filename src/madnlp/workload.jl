...
@setup_workload begin
    ...
    opts_ptr_vec = Vector{Ptr{MadNLPOptsDict}}([C_NULL])
    opts_ptr = opts_ptr_vec[1]
    opts_ptr_ptr = pointer(opts_ptr_vec)
    nlp_ptr_vec = Vector{Ptr{CNLPModel{Cdouble}}}([C_NULL])
    nlp_ptr = nlp_ptr_vec[1]
    nlp_ptr_ptr = pointer(nlp_ptr_vec)
    solver_ptr_vec = Vector{Ptr{MadNLPSolver}}([C_NULL])
    ...
    println(libmad_refs)
    println(opts_ptr_vec)
    println(nlp_ptr_vec)
    println(solver_ptr_vec)

    @compile_workload begin


        _name = "aname"
        nlpmodel_cpu_create(nlp_ptr_ptr,
        ...
        nlp_ptr = nlp_ptr_vec[1]
        madnlpoptions_create_options_struct(opts_ptr_ptr)
        opts_ptr = opts_ptr_vec[1]
        ...
        madnlpoptions_set_float64_option(opts_ptr, unsafe_convert(Cstring,_tol), Cdouble(1e-6))
        madnlpoptions_set_int64_option(opts_ptr, unsafe_convert(Cstring,_max_iter), 2000)
        #madnlpoptions_set_int64_option(opts_ptr, unsafe_convert(Cstring,_print_level), 1)
        madnlpoptions_set_string_option(opts_ptr, unsafe_convert(Cstring,_callback), unsafe_convert(Cstring,_SparseCallback))
        madnlpoptions_set_bool_option(opts_ptr, unsafe_convert(Cstring,_hessian_constant), false)

        madnlp_create_solver(solver_ptr_ptr, nlp_ptr, opts_ptr)
        solver_ptr = solver_ptr_vec[1]
